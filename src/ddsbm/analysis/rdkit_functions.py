...
class BasicMolecularMetrics(object):
    def __init__(self, dataset_info, train_smiles=None):
        ...
        # NOTE : Dummy Atom is represented by 'X'
        self.no_dummy = False
        if "X" in self.atom_decoder:
            self.dummy_index = self.atom_decoder.index("X")
        elif dataset_info.name == "MOSES":
            self.no_dummy = True
        else:
            raise ValueError("Dummy atom not found in atom decoder")

        # Retrieve dataset smiles only for qm9 currently.
        self.dataset_smiles_list = train_smiles

    def delete_dummy_atoms(self, atom_types, edge_types):
        """
        Remove dummy atoms from the graph
        atom_types: (b, n, af)
        edge_types: (b, n, n, ef)
        """
        if not self.no_dummy:
            mask = atom_types != self.dummy_index
    ...
    def compute_validity(self, generated):
        ...
        for graph in generated:
            ...
            atom_types, edge_types = self.delete_dummy_atoms(atom_types, edge_types)
...
def mol2smiles(mol, verbose=False):
    try:
        Chem.SanitizeMol(mol)
        # NOTE: ADDED Since sometimes smiles are not converted to mol due to Kekulize Error
        smi_from_mol = Chem.MolToSmiles(mol)
        mol = Chem.MolFromSmiles(smi_from_mol)
        if mol is None:
            if verbose:
                print(f"MolFromSmiles failed on {smi_from_mol}")
            return None
...
def build_molecule_with_atom_map(atom_types, edge_types, atom_decoder, verbose=False):
    if verbose:
        print("building new molecule")
    edge_types = torch.where(edge_types < 5, edge_types, torch.zeros_like(edge_types))
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info):
    ...
    if not dataset_info.remove_h:
        ...
        for i, mol in enumerate(molecule_list):
            ...
            # NOTE : Hardcoded to remove dummy atoms
            # atom_types, edge_types = graph
            mask = atom_types != 12
            atom_types = atom_types[mask]
            edge_types = edge_types[mask][:, mask]
