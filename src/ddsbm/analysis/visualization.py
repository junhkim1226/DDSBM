...
class MolecularVisualization:
    ...
    def mol_from_graphs(self, node_list, adjacency_matrix):
        ...
        # NOTE : Remove dummy nodes
        mask = node_list != atom_decoder.index("X")
        node_list = node_list[mask]
        adjacency_matrix = adjacency_matrix[mask][:, mask]
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix):
        ...
        # NOTE : Except the first molecule
        # NOTE : Number of atoms are not same in all the molecules
        ...
        # draw grid image
        try:
            img = Draw.MolsToGridImage(mols, molsPerRow=10, subImgSize=(300, 300))
            img.save(
...
class NonMolecularVisualization:
    def delete_dummy_nodes(self, node_list, adjacency_matrix):
        mask = node_list != 0
        node_list = node_list[mask]
        adjacency_matrix = adjacency_matrix[mask][:, mask]
    ...
    def visualize_non_molecule(
        ...
        if ref_graph is None:
            ...
            # Set node colors based on the eigenvectors
            w, U = np.linalg.eigh(nx.normalized_laplacian_matrix(ref_graph).toarray())
            vmin, vmax = np.min(U[:, 1]), np.max(U[:, 1])
            m = max(np.abs(vmin), vmax)
            vmin, vmax = -m, m
    ...
    def visualize(
        ...
        for i in range(num_graphs_to_visualize):
            try:
                ...
                if not dataset == "enzyme" and (graphs[i][0] != 0).sum() != 0:
                    # print(f"Graph {i} has dummy nodes")
                    nodes, adj = self.delete_dummy_nodes(
                        graphs[i][0].numpy(), graphs[i][1].numpy()
                    )
                else:
                    nodes, adj = graphs[i][0].numpy(), graphs[i][1].numpy()
                graph = self.to_networkx(nodes, adj)
    ...
    def visualize_chain(self, path, nodes_list, adjacency_matrix, dataset=None):
        # convert graphs to networkx
        try:
            graphs = []
            for i in range(nodes_list.shape[0]):
                # FIXME : NO dummy
                # if not dataset == "enzyme" and (nodes_list[i] != 0).sum() != 0:
                #     print(f"Graph {i} has dummy nodes")
                #     nodes, adj = self.delete_dummy_nodes(nodes_list[i], adjacency_matrix[i])
                # else:
                if True:
                    nodes, adj = nodes_list[i], adjacency_matrix[i]
                graphs.append(self.to_networkx(nodes, adj))
                # graphs = [
            ...
            _final_graph = graphs[-1]
            final_pos = nx.spring_layout(_final_graph, seed=0)
            ...
            for frame in range(num_frams):
                file_name = os.path.join(path, "fram_{}.png".format(frame))
                # self.visualize_non_molecule(graph=graphs[frame],
                #                             pos=final_pos,
                #                             path=file_name)
                if frame == num_frams - 1:
                    last = True
                else:
                    last = False
