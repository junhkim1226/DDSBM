...
from ddsbm.analysis.dist_helper import (
    compute_mmd,
    disc,
    emd,
    gaussian,
...
def eval_fraction_unique_non_isomorphic_valid(
    ...
    frac_unique_non_isomorphic_valid = (
        count_valid / float(len(fake_graphs))
...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, datamodule, compute_emd, metrics_list, direction):
        super().__init__()

        self.train_graphs = self.loader_to_nx(
            datamodule.train_dataloader(), direction=direction
        )
        self.val_graphs = self.loader_to_nx(
            datamodule.val_dataloader(), direction=direction
        )
        self.test_graphs = self.loader_to_nx(
            datamodule.test_dataloader(), direction=direction
        )
        self.num_graphs_test = len(self.test_graphs)
        self.num_graphs_val = len(self.val_graphs)
        self.compute_emd = compute_emd
        self.metrics_list = metrics_list
        print(f"Debug -- metrics_list: {metrics_list}")

    def loader_to_nx(self, loader, direction):
        networkx_graphs = []
        for i, batch in enumerate(loader):
            data_list = batch.to_data_list()
            for j, data in enumerate(data_list):
                if data.__class__.__name__ == "HeteroData":
                    # NOTE : Deprecated
                    ...
                    if direction == "backward":
                        data = Data(
                            x=data.x_0_p,
                            edge_index=data.edge_index_0_p,
                            edge_attr=data.edge_attr_0_p,
                            num_nodes=len(data.x_0_p),
                        )
                    else:
                        data = Data(
                            x=data.x_T_p,
                            edge_index=data.edge_index_T_p,
                            edge_attr=data.edge_attr_T_p,
                            num_nodes=len(data.x_T_p),
                        )
...
class Comm20SamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule, direction):
        super().__init__(
