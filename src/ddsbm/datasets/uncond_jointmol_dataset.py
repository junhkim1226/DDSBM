...
def process_mol(mol, smi, max_num_nodes, atom_decoder):
    ...
    # NOTE : Only use kekulized version X, E (junhkim)
    Chem.Kekulize(mol)
    ...
    # NOTE : Do Not Pad in unconditional generation
    ...
    for bond in mol.GetBonds():
        ...
        assert bonds[bond.GetBondType()] != 3, "Aromatic Detected"
...
class JointMolDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, line in target_df.iterrows():
            ...
            smi_T = line["PRB-SMI"]
            mol_T = Chem.MolFromSmiles(smi_T)
            Chem.SanitizeMol(mol_T)
