...
from ddsbm.utils import PlaceHolder
...
def cosine_alpha_schedule_discrete(timesteps, min_alpha, s=0.008):
    """Cosine schedule"""
    steps = (timesteps + 2) // 2
    x = np.linspace(0, steps, steps)

    # alphas = 0.5 * (np.cos(0.5 * np.pi * ((2 * x / steps) + s) / (1 + s)) + 1)  * (1-min_alphas) + min_alphas
    alphas = (
        np.cos(0.5 * np.pi * ((x / steps) + s) / (1 + s)) ** 2 * (1 - min_alpha)
        + min_alpha
    )
    # repeat alphas to match the number of timesteps
    alphas = np.concatenate([alphas, np.flip(alphas)])
    # alphas = np.cos(0.5 * np.pi * ((2 * x / steps) + s) / (1 + s))
    return alphas.squeeze()


def sigmoid_beta_schedule_discrete(beta_start, beta_end, timesteps, s=0.008):
    """Sigmoid schedule"""
...
def sample_discrete_features(probX, probE, node_mask):
    ...
    # Sample E
    # print(f"debug] (sample_discrete_features)\t\n probE: {probE}")
...
def compute_marginal_distribution(M_0, M_T, Qtb_M, QTb_M, QuTb_M):
    """M: X or E
    Compute xT @ Q_uTb.T * x0 @ Qtb / x0 @ QTb @ xT.T
    """
    # Flatten feature tensors
    M_0 = M_0.flatten(start_dim=1, end_dim=-2).to(
        torch.float32
    )  # (bs, N, d) with N = n or n * n
    M_T = M_T.flatten(start_dim=1, end_dim=-2).to(torch.float32)  # same

    QuTb_M = torch.transpose(QuTb_M, -2, -1)  # (bs, dT, dt)

    left_term = M_T @ QuTb_M  # (bs, N, dt)
    right_term = M_0 @ Qtb_M  # (bs, N, dt)
    product = left_term * right_term  # (bs, N, dt)

    denom = M_0 @ QTb_M  # (bs, N, d0) @ (bs, d0, dT) = (bs, N, dT)
    denom = (denom * M_T).sum(dim=-1)  # (bs, N, dT) * (bs, N, dT) + sum = (bs, N)
    denom[denom == 0] = 1e-6
...
def compute_forward_distribution(M_t, M_T, Qu_M, QuTb_M, QvTb_M):
    """M: X or E
    Compute xT @ QvTb.T * xt @ Qu / xt @ QuTb @ xT.T
    """
    # Flatten feature tensors
    M_t = M_t.flatten(start_dim=1, end_dim=-2).to(
        torch.float32
    )  # (bs, N, d) with N = n or n * n
    M_T = M_T.flatten(start_dim=1, end_dim=-2).to(torch.float32)  # same

    QvTb_M = torch.transpose(QvTb_M, -2, -1)  # (bs, dT, d_t+1)

    left_term = M_T @ QvTb_M  # (bs, N, d_t+1)
    right_term = M_t @ Qu_M  # (bs, N, d_t+1)
    product = left_term * right_term  # (bs, N, d_t+1)

    denom = M_t @ QuTb_M  # (bs, N, dt) @ (bs, dt, dT) = (bs, N, dT)
    denom = (denom * M_T).sum(dim=-1)  # (bs, N, dT) * (bs, N, dT) + sum = (bs, N)
    denom[denom == 0] = 1e-6

    prob = product / denom.unsqueeze(-1)  # (bs, N, d_t+1)

    return prob


def compute_backward_distribution(M_t, M_0, Qt_M, Qsb_M, Qtb_M):
    """M: X or E
    ...
    M_0 = M_0.flatten(start_dim=1, end_dim=-2).to(torch.float32)  # same

    Qt_M = torch.transpose(Qt_M, -2, -1)  # (bs, dt, d_t-1)

    left_term = M_t @ Qt_M  # (bs, N, d_t-1)
    right_term = M_0 @ Qsb_M  # (bs, N, d_t-1)
    product = left_term * right_term  # (bs, N, d_t-1)
...
def compute_posterior_distribution(M, M_t, Qt_M, Qsb_M, Qtb_M):
    ...
    # Flatten feature tensors
    # (bs, N, d) with N = n or n * n
    # NOTE : If M is edge feature, transition matrix should be (bs, n*n, d_t-1, dt)
    if len(M.shape) == 4:
        Qt_M = Qt_M.flatten(start_dim=1, end_dim=2).to(
            torch.float32
        )  # (bs, N, d_t-1, dt)
        Qsb_M = Qsb_M.flatten(start_dim=1, end_dim=2).to(
            torch.float32
        )  # (bs, N, d0, d_t-1)
        Qtb_M = Qtb_M.flatten(start_dim=1, end_dim=2).to(
            torch.float32
        )  # (bs, N, d0, dt)
    ...
    Qt_M_T = Qt_M.transpose(-1, -2)  # (bs, N, dt, d_t-1)

    left_term = (
        M_t.unsqueeze(2) @ Qt_M_T
    )  # (bs, N, 1, d_t-1) <- (bs, N, 1, dt @ bs, N, dt, d_t-1)
    right_term = (
        M.unsqueeze(2) @ Qsb_M
    )  # (bs, N, 1, d_t-1) <- (bs, N, 1, d0 @ bs, N, d0, d_t-1)
    product = left_term * right_term  # (bs, N, 1, d_t-1)

    denom = M.unsqueeze(2) @ Qtb_M  # (bs, N, 1, d0) @ (bs, N, d0, dt) = (bs, N, 1, dt)

    # (bs, N, d) * (bs, N, d) + sum = (bs, N)
    # (bs, N, 1, dt) * (bs, N, 1, dt)  + sum = (bs, N, 1)
    denom = (denom * M_t.unsqueeze(2)).sum(dim=-1)  # (bs, N, 1)
    # denom = product.sum(dim=-1)
    # denom[denom == 0.] = 1

    # prob = product / denom.unsqueeze(-1)    # (bs, N, d)
    denom[denom == 0] = 1e-6
    prob = product.squeeze(2) / denom  # (bs, N, d)
