...
class TestLossDiscrete(nn.Module):
    ...
    def forward(
        ...
        if log:
            to_log = {
                f"{self.name}_loss/batch_CE": (loss_X + loss_E + loss_y).detach(),
                f"{self.name}_loss/X_CE": (
                    self.node_loss.compute() if true_X.numel() > 0 else -1
                ),
                f"{self.name}_loss/E_CE": (
                    self.edge_loss.compute() if true_E.numel() > 0 else -1
                ),
                f"{self.name}_loss/y_CE": (
                    self.y_loss.compute() if true_y.numel() > 0 else -1
                ),
            }
            if wandb.run:
    ...
    def log_epoch_metrics(self):
        ...
        to_log = {
            f"{self.name}_epoch/x_CE": epoch_node_loss,
